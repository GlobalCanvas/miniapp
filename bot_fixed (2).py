                        ...
                        if resp.status == 200:
                            ...
                            if match:
                                ...
                                if last_void_raw.get(site_name) != raw_time_str:
                                    ...
                                    utc_display = dt_utc.strftime("%d.%m.%Y at %H:%M:%S UTC")
                                        ...
                                        if chat_data.get("site") == site_name:
                                            try:
                                                text = (
                                                    ...
                                                    f"🕐 <b>UTC:</b> <code>{utc_display}</code>\n"
...
async def cmd_start(message: Message):
    ...
    text = (
        ...
        "/madd - Додати медаль (адмін)\n"
        "/mdel - Видалити медаль (адмін)\n\n"
...
async def cmd_upload(message: Message):
    """Команда /upload - завантаження шаблону"""
    if not await is_admin_or_creator(message):
        await message.answer("❌ Тільки для адмінів!")
        return
    
    # Перевіряємо чи є фото (або в самому повідомленні, або у відповіді)
    ...
    if not photo_message.photo:
        await message.answer(
            "❌ Використання: /upload X Y\n"
            "Варіант 1: Прикріпіть зображення до повідомлення з командою\n"
            "Варіант 2: Відповідайте командою на повідомлення з фото"
        )
        return
    ...
    # Спочатку пробуємо з основного повідомлення (якщо це reply)
    if message.text:
        args = message.text.split()[1:]
    
    # Якщо немає аргументів і фото має підпис, беремо з підпису
    if not args and photo_message.caption:
        caption_parts = photo_message.caption.split()
        # Шукаємо числа в підписі (можуть бути після /upload або просто числа)
        for i, part in enumerate(caption_parts):
            if part.lstrip('-').isdigit():
                # Знайшли перше число, беремо його і наступне
                if i + 1 < len(caption_parts) and caption_parts[i + 1].lstrip('-').isdigit():
                    args = [caption_parts[i], caption_parts[i + 1]]
                    break
    
    if len(args) < 2:
        await message.answer(
            "❌ Використання: /upload X Y\n"
            "Приклад 1: /upload 1000 2000 (з прикріпленим зображенням)\n"
            "Приклад 2: Відповісти на фото командою /upload 1000 2000\n"
            "Приклад 3: Надіслати фото з підписом '1000 2000' і відповісти /upload"
        )
        return
    ...
    try:
        ...
        await message.answer("❌ Координати мають бути числами!")
        return
    ...
    try:
        ...
        # Завантажуємо фото
        photo = photo_message.photo[-1]  # Найбільший розмір
...
async def cmd_topfac(message: Message):
    ...
    try:
        ...
        faction_aliases = {
            "ukrway": ["ukrway", "ukwy", "ukrwayreborn"],
            "ukwy": ["ukrway", "ukwy", "ukrwayreborn"],
        }
        ...
        # DEBUG: Лог для відлагодження
        print(f"DEBUG: Шукаємо фракцію: {faction_name}")
        print(f"DEBUG: Normalized: {search_normalized}")
        print(f"DEBUG: Search variants: {search_variants}")
        ...
        for player in data.get("ranking", []):
            ...
            if fac_info and len(fac_info) >= 2:
                ...
                for variant in search_variants:
                    if (variant in tag_normalized or 
                        variant in name_normalized or
                        tag_normalized in variant or
                        name_normalized in variant):
                        match_found = True
                        # DEBUG
                        print(f"DEBUG: MATCH! Player: {player.get('name')}, Faction: {name} [{tag}]")
                        break
        ...
        if not faction_players:
            ...
            for player in data.get("ranking", []):
                ...
                if fac_info and len(fac_info) >= 2:
                    tag = str(fac_info[0])
                    name = str(fac_info[1])
                    all_factions.add(f"{name} [{tag}]")
            
            factions_list = "\n".join(sorted(list(all_factions))[:20])
            
            await status_msg.edit_text(
                f"❌ Гравців фракції <code>{faction_name}</code> не знайдено!\n\n"
                f"💡 Доступні фракції (перші 20):\n\n"
                f"<code>{factions_list}</code>",
                parse_mode="HTML"
            )
            return
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(site["void_url"], timeout=10) as resp:
                if resp.status == 200:
                    ...
                    utc_time = time_match.group(0) if time_match else "Н/Д"
                    ...
                    if is_active:
                        text += f"⏰ <b>Почався:</b> <code>{utc_time}</code>\n"
                        if next_void_in != "Н/Д":
                            text += f"⏳ <b>Закінчиться через:</b> <code>{next_void_in}</code>\n"
                    else:
                        text += f"⏰ <b>Наступний войд:</b> <code>{utc_time}</code>\n"
                        if next_void_in != "Н/Д":
...
async def cmd_medals(message: Message):
    ...
    if user_id not in medals_db or not medals_db[user_id].get("medals"):
        await message.answer("У вас ще немає медалей.")
        return
    
    text = f"🏆 <b>Медалі: {message.from_user.first_name}</b>\n\n"
    
    for i, m in enumerate(medals_db[user_id]["medals"], 1):
        stars = "⭐" * m["weight"]
        text += f"{i}. <b>{m['name']}</b>\n{stars}\n\n"
...
async def cmd_madd(message: Message):
    ...
    if len(args) < 3:
        await message.answer(
            "❌ Використання: /madd <user_id> <вага> <назва_медалі>\n"
            "Приклад: /madd 123456789 5 Кращий Піксель Артист"
        )
        return
    ...
    try:
        ...
        if user_id not in medals_db:
            medals_db[user_id] = {"username": "Користувач", "medals": []}
        
        medals_db[user_id]["medals"].append({
            "name": medal_name,
            "weight": weight,
...
async def cmd_mdel(message: Message):
    ...
    if len(args) < 2:
        await message.answer(
            "❌ Використання: /mdel <user_id> <номер_медалі>\n"
            "Приклад: /mdel 123456789 1"
        )
        return
    ...
    try:
        ...
        if user_id not in medals_db or not medals_db[user_id].get("medals"):
            await message.answer("❌ У користувача немає медалей!")
            return
        
        if index < 1 or index > len(medals_db[user_id]["medals"]):
            await message.answer("❌ Невірний номер медалі!")
            return
        
        removed = medals_db[user_id]["medals"].pop(index - 1)
        save_all()
        
        text = f"✅ <b>Медаль видалено:</b> {removed['name']}"
        await message.answer(text, parse_mode="HTML")
