...
from minimap_module import upload_mini, upload_mini_handler, check_mini, get_mini, debug_mini, sitemini, UploadMiniStates

# ================== OPTIMIZATIONS ==================
# ✅ ОПТИМІЗОВАНО: Додано async compare_images_async() для запобігання
#    блокуванню event loop при обробці великих зображень (256MB+)
# ✅ Використовується run_in_executor для виконання важких обчислень
#    в окремому потоці без блокування основного циклу бота
...
VOID_CHATS = {}
USER_CONNECTIONS = {}
LAST_CHECK_RESULTS = {}  # Зберігає результати останнього check для кожного чату
...
# Одноразові коди доступу: {code: expires_timestamp}
PENDING_CODES = {}
# Авторизовані чати: {chat_id: {"name": ..., "added_at": ...}}
ALLOWED_CHATS = {}
...
# ================== PERSISTENCE ==================
def load_data():
    global TEMPLATE_DATA, medals_db, VOID_CHATS, USER_CONNECTIONS, LAST_CHECK_RESULTS, ALLOWED_CHATS
    try:
        ...
        if os.path.exists(LAST_CHECK_FILE):
            with open(LAST_CHECK_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
                LAST_CHECK_RESULTS = {int(k): v for k, v in data.items()}
        
        if os.path.exists(ALLOWED_CHATS_FILE):
            with open(ALLOWED_CHATS_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
                ALLOWED_CHATS = {int(k): v for k, v in data.items()}
...
def save_all():
    try:
        ...
        with open(LAST_CHECK_FILE, 'w', encoding='utf-8') as f:
            json.dump(LAST_CHECK_RESULTS, f, ensure_ascii=False, indent=4)
        
        with open(ALLOWED_CHATS_FILE, 'w', encoding='utf-8') as f:
            json.dump(ALLOWED_CHATS, f, ensure_ascii=False, indent=4)
    except Exception as e:
...
def is_chat_allowed(chat_id: int) -> bool:
    """Перевіряє чи має чат доступ до бота"""
    return chat_id in ALLOWED_CHATS or chat_id == CREATOR_ID


# ================== ACCESS CONTROL COMMANDS ==================
async def cmd_gencode(message: Message):
    """/gencode — генерує одноразовий код доступу (тільки власник в ЛС)"""
    if message.from_user.id != CREATOR_ID:
        return
    if message.chat.type != "private":
        await message.answer("⚠️ Команда доступна тільки в особистих повідомленнях.")
        return

    # Видаляємо протерміновані коди
    now = time.time()
    expired = [c for c, exp in PENDING_CODES.items() if exp < now]
    for c in expired:
        del PENDING_CODES[c]

    code = secrets.token_hex(4).upper()  # напр. A3F7B2C1
    PENDING_CODES[code] = now + 600  # дійсний 10 хвилин

    await message.answer(
        f"🔑 <b>Одноразовий код доступу:</b>\n\n"
        f"<code>/code {code}</code>\n\n"
        f"⏳ Дійсний <b>10 хвилин</b>.\n"
        f"Надішли цю команду в потрібному чаті.",
        parse_mode="HTML"
    )


async def cmd_code(message: Message):
    """/code XXXX — активує доступ до бота в поточному чаті"""
    args = message.text.split()[1:] if message.text else []

    if not args:
        await message.answer("❌ Використання: /code XXXXXXXX")
        return
    ...
    if code not in PENDING_CODES:
        await message.answer("❌ Невірний або вже використаний код.")
        return

    if PENDING_CODES[code] < now:
        del PENDING_CODES[code]
        await message.answer("❌ Код прострочений. Попроси власника згенерувати новий /gencode.")
        return

    # Код вірний — видаляємо (одноразовий)
    del PENDING_CODES[code]

    chat_id = message.chat.id
    chat_name = message.chat.title or message.chat.full_name or str(chat_id)
    ...
    await message.answer(
        f"✅ <b>Доступ активовано!</b>\n\n"
        f"🏠 Чат: <b>{chat_name}</b>\n"
        f"🤖 Бот тепер доступний у цьому чаті.",
        parse_mode="HTML"
    )


async def cmd_revoke(message: Message):
    """/revoke [chat_id] — відкликати доступ (тільки власник в ЛС)"""
    if message.from_user.id != CREATOR_ID:
        return
    if message.chat.type != "private":
        await message.answer("⚠️ Команда доступна тільки в особистих повідомленнях.")
    ...
    if not args:
        # Показуємо список дозволених чатів
        if not ALLOWED_CHATS:
            await message.answer("📋 Немає авторизованих чатів.")
            return
        
        lines = ["📋 <b>Авторизовані чати:</b>\n"]
        for cid, info in ALLOWED_CHATS.items():
            lines.append(f"• <b>{info['name']}</b>\n  ID: <code>{cid}</code>\n  Додано: {info['added_at']}")
        lines.append("\nВикористання: /revoke &lt;chat_id&gt;")
    ...
    try:
        ...
        await message.answer("❌ Невірний chat_id. Використай /revoke щоб побачити список.")
        return

    if target_id not in ALLOWED_CHATS:
        await message.answer(f"❌ Чат <code>{target_id}</code> не знайдено в списку.", parse_mode="HTML")
        return

    chat_name = ALLOWED_CHATS[target_id]["name"]
    del ALLOWED_CHATS[target_id]
    save_all()

    await message.answer(
        f"✅ Доступ відкликано для чату <b>{chat_name}</b> (<code>{target_id}</code>).",
        parse_mode="HTML"
    )
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(site_data["void_url"], timeout=15) as resp:
                        if resp.status == 200:
                            ...
                            # Якщо войд не оголошено — ігноруємо
                            if "Void has either won" in content or "hasn't been announced" in content:
                                continue
...
def compare_images_sync(base_img, template_img, tolerance=35, debug=False):
    """Порівняння зображень - показує кольори шаблону для правильних пікселів"""
    ...
    for y in range(h):
        for x in range(w):
            ...
            if distance <= tolerance:
                matched += 1
                if debug:
                    # Показуємо оригінальний колір з шаблону замість зеленого
                    op[x, y] = tp[x, y]
            else:
                wrong += 1
                if debug:
...
async def compare_images_async(base_img, template_img, tolerance=35, debug=False):
    """Асинхронне порівняння зображень - запобігає блокуванню event loop"""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, compare_images_sync, base_img, template_img, tolerance, debug)
...
async def cmd_start(message: Message):
    ...
    text = (
        ...
        "/debug_mini - Переглянути карту MiniMap\n"
        ...
        "<b>🔐 Доступ (тільки власник):</b>\n"
        "/gencode - Згенерувати код доступу\n"
        "/revoke - Переглянути/відкликати доступ"
    )
...
async def cmd_check(message: Message):
    ...
    try:
        ...
        with Image.open(info["path"]) as t_img:
            result, p, m, t, missing, wrong = await compare_images_async(base, t_img, tolerance=35, debug=True)
        ...
        # Перевіряємо чи є попередній результат
        diff_text = ""
        if chat_id in LAST_CHECK_RESULTS:
            last_matched = LAST_CHECK_RESULTS[chat_id].get("matched", 0)
            diff = m - last_matched
            
            if diff > 0:
                diff_text = f"\n📈 <b>Змінено:</b> <code>+{diff:,}</code> px 🟢"
            elif diff < 0:
                diff_text = f"\n📉 <b>Змінено:</b> <code>{diff:,}</code> px 🔴"
            else:
                diff_text = f"\n➡️ <b>Змінено:</b> <code>0</code> px"
        
        # Зберігаємо поточний результат
        LAST_CHECK_RESULTS[chat_id] = {
            "matched": m,
            "total": t,
            "percent": p,
...
def _normalize_factions(data) -> list:
    """
    Нормалізує відповідь API фракцій до списку словників.
    API може повертати:
      - list  [{"id":1,...}, ...]   → повертаємо як є
      - dict  {"id":1, "members":[...]}  → одна фракція, обертаємо у список
      - dict  {"1": {...}, "2": {...}}   → словник фракцій, беремо values()
    """
    if isinstance(data, list):
        return data
    if isinstance(data, dict):
        # Якщо є ключ "members" — це одна фракція
        if "members" in data:
            return [data]
        # Інакше — словник фракцій по ключах
...
async def cmd_connect(message: Message):
    ...
    try:
        ...
        # ── PIXMAP: шукаємо через api_factions ──
        if site_name == "pixmap" and "api_factions" in SITES[site_name]:
            async with aiohttp.ClientSession() as session:
                async with session.get(SITES[site_name]["api_factions"], timeout=15) as resp:
                    if resp.status != 200:
                        await status_msg.edit_text("❌ Не вдалося завантажити список фракцій!")
                        return
                    factions_data = _normalize_factions(await resp.json())

            for faction in factions_data:
                for member in faction.get("members", []):
                    if normalize_name(member.get("name", "")) == search_name:
                        player_data = {
                            "name": member["name"],
                            "px": member.get("totalPixels", 0),
                            "daily": member.get("dailyPixels", 0),
                        }
                        faction_name = f"{faction.get('name', '')} [{faction.get('id', '')}]"
                        faction_role = member.get("role")
                        break
                if player_data:
                    break
            ...
            if not player_data:
                await status_msg.edit_text(
                    f"❌ Гравця <b>{nickname}</b> не знайдено у жодній фракції на <b>{site_name}</b>!\n\n"
                    f"💡 Переконайтесь що нікнейм вірний",
                    parse_mode="HTML"
                )
                return

        # ── Інші сайти: шукаємо через api_ranking ──
        else:
            async with aiohttp.ClientSession() as session:
        ...
        if faction_role:
            text += f"🎖️ <b>Роль:</b> <code>{faction_role}</code>\n"
...
async def cmd_profile(message: Message):
    ...
    try:
        ...
        # ── PIXMAP: шукаємо у фракціях ──
        if site_name == "pixmap" and "api_factions" in SITES[site_name]:
            async with aiohttp.ClientSession() as session:
                async with session.get(SITES[site_name]["api_factions"], timeout=15) as resp:
                    if resp.status != 200:
                        await status_msg.edit_text("❌ Не вдалося завантажити дані!")
                        return
                    factions_data = _normalize_factions(await resp.json())
            ...
            for faction in factions_data:
                for member in faction.get("members", []):
                    if normalize_name(member.get("name", "")) == search_name:
                        player_data = member
                        faction_name = faction.get("name", "Н/Д")
                        faction_role = member.get("role")
                        break
                if player_data:
                    break
            ...
            if not player_data:
                await status_msg.edit_text(
                    f"❌ Гравця <code>{nickname}</code> не знайдено у фракціях!\n"
                    f"💡 Спробуйте перепідключитися через /connect",
                    parse_mode="HTML"
                )
                return

            total_pixels = player_data.get("totalPixels", 0)
            daily_pixels = player_data.get("dailyPixels", 0)

            text = (
                f"👤 <b>Профіль: {player_data.get('name', nickname)}</b>\n\n"
                f"🌐 <b>Сайт:</b> <code>{site_name}</code>\n"
                f"🎨 <b>Всього пікселів:</b> <code>{total_pixels:,}</code>\n"
...
async def cmd_topfac(message: Message):
    ...
    if len(args) < 1:
        await message.answer(
            "❌ Використання:\n"
            "• /topfac <назва_фракції>  (pixelya/pixunivers)\n"
            "• /topfac <назва>/<id>  (pixmap, напр: ukrlirn/530)"
        )
        return
    
    faction_query = " ".join(args)
    chat_id = message.chat.id
    site = TEMPLATE_DATA.get(chat_id, {}).get("site", "pixelya")
    
    try:
        status_msg = await message.answer(f"🔍 Пошук фракції '{faction_query}'...")

        # ══════════════════════════════════════════
        # PIXMAP: окремий API фракцій
        # ══════════════════════════════════════════
        if site == "pixmap" and "api_factions" in SITES[site]:
            # Парсимо аргумент: може бути "назва/id" або просто "назва" або просто "id"
            faction_id = None
            faction_name_query = faction_query

            if "/" in faction_query:
                parts = faction_query.rsplit("/", 1)
                faction_name_query = parts[0].strip()
                try:
                    faction_id = int(parts[1].strip())
                except ValueError:
                    pass
            else:
                try:
                    faction_id = int(faction_query.strip())
                    faction_name_query = None
                except ValueError:
                    pass

            async with aiohttp.ClientSession() as session:
                async with session.get(SITES[site]["api_factions"], timeout=15) as resp:
                    if resp.status != 200:
                        await status_msg.edit_text("❌ Не вдалося завантажити список фракцій!")
                        return
                    factions_data = _normalize_factions(await resp.json())

            # Шукаємо фракцію по ID або назві
            target_faction = None
            ...
            if target_faction is None and faction_name_query:
                norm_query = faction_name_query.lower().strip()
                for fac in factions_data:
                    fac_name_norm = str(fac.get("name", "")).lower()
                    if norm_query in fac_name_norm or fac_name_norm.startswith(norm_query):
                        target_faction = fac
                        break
            ...
            if target_faction is None:
                # Показуємо доступні
                available = "\n".join([
                    f"• <code>{f['name']}</code> (ID: {f['id']}) — {f.get('memberCount', '?')} членів"
                    for f in sorted(factions_data, key=lambda x: x.get("totalPixels", 0), reverse=True)[:15]
                ])
                await status_msg.edit_text(
                    f"❌ Фракцію <code>{faction_query}</code> не знайдено!\n\n"
                    f"💡 Формат: /topfac назва/<b>id</b>\n"
                    f"Наприклад: /topfac ukrlirn/530\n\n"
                    f"<b>Доступні фракції (топ-15):</b>\n{available}",
                    parse_mode="HTML"
                )
                return

            # Будуємо текст
            fac_name = target_faction.get("name", "Невідома")
            fac_id = target_faction.get("id", "?")
            fac_total = target_faction.get("totalPixels", 0)
            fac_daily = target_faction.get("dailyPixels", 0)
            fac_owner = target_faction.get("owner", {}).get("name", "?")
            fac_desc = target_faction.get("description", "")
            members = target_faction.get("members", [])

            # Сортуємо членів по totalPixels
            members_sorted = sorted(members, key=lambda m: m.get("totalPixels", 0), reverse=True)

            text = (
                f"🚩 <b>{fac_name}</b> (ID: {fac_id})\n"
                f"👑 <b>Власник:</b> <code>{fac_owner}</code>\n"
                f"👥 <b>Членів:</b> {target_faction.get('memberCount', len(members))}\n"
                f"🎨 <b>Всього пікселів:</b> <code>{fac_total:,}</code>\n"
                f"📊 <b>Сьогодні:</b> <code>{fac_daily:,}</code>\n"
            )
            if fac_desc:
                desc_short = fac_desc[:80].replace("\n", " ")
                text += f"📝 <i>{desc_short}{'...' if len(fac_desc) > 80 else ''}</i>\n"

            text += f"\n<b>Топ гравців на {site}:</b>\n\n"

            for i, member in enumerate(members_sorted[:15], 1):
                name = member.get("name", "Unknown")
                total = member.get("totalPixels", 0)
                daily = member.get("dailyPixels", 0)
                ...
                text += (
                    f"{medal} <b>{i}. {name}</b> {role_icon}\n"
                    f"📅 Сьогодні: <code>{daily:,}</code> | "
                    f"📊 Всього: <code>{total:,}</code>\n\n"
                )
        ...
        search_normalized = normalize(faction_query)
        ...
        if not faction_players:
            ...
            error_msg = f"❌ Гравців фракції <code>{faction_query}</code> не знайдено!\n\n"
            ...
            async with session.get(site["void_url"], timeout=10) as resp:
                if resp.status == 200:
                    ...
                    if 'application/json' in content_type:
                        ...
                        # Перевіряємо чи войд не оголошено / виграно
                        if "Void has either won" in content or "hasn't been announced" in content or "not been announced" in content:
                            await message.answer(
                                f"🌌 <b>Статус Войду: {site_name}</b>\n\n"
                                f"⚫ <b>Войд не оголошено або вже виграно.</b>\n\n"
                                f"💡 Слідкуй за сповіщеннями через /void",
                                parse_mode="HTML"
                            )
                            return
                        ...
                        voidinfo_match = re.search(r'Void info: (.*?)(?=\n|$)', content)
                        ...
                        voidinfo = voidinfo_match.group(1).strip() if voidinfo_match else "Н/Д"
...
async def cmd_upload_mini(message: Message, state: FSMContext):
    """Команда /upload_mini"""
    await upload_mini(message, message.bot, state)
...
async def cmd_debug_mini(message: Message, chunk_downloader: ChunkDownloader):
    """Команда /debug_mini - показати поточний стан MiniMap"""
    await debug_mini(message, message.bot, chunk_downloader)
...
async def main():
    ...
    # Створюємо wrapper функції для check_mini з chunk_downloader
    async def cmd_check_mini_wrapper(message: Message):
        await check_mini(message, message.bot, chunk_downloader)

    async def cmd_debug_mini_wrapper(message: Message):
        await debug_mini(message, message.bot, chunk_downloader)
    ...
    class AccessMiddleware(BaseMiddleware):
        async def __call__(
            ...
            if event.from_user and event.from_user.id == CREATOR_ID:
                return await handler(event, data)
            # Приватні чати власника — дозволено вище
            # Дозволені команди без авторизації
            text = event.text or ""
            if (text.startswith("/code") or 
                text.startswith("/start") or
                text.startswith("/gencode") or
                text.startswith("/revoke")):
                return await handler(event, data)
            # Перевірка чи чат авторизований
            if not is_chat_allowed(event.chat.id):
                await event.answer(
                    "🔒 <b>Доступ обмежено.</b>\n\n"
                    "Цей бот працює тільки в авторизованих чатах.\n"
                    "Попроси власника надати код доступу та введи:\n"
                    "<code>/code XXXXXXXX</code>",
                    parse_mode="HTML"
                )
                return
            return await handler(event, data)
    ...
    # Реєстрація команд доступу
    dp.message.register(cmd_gencode, Command("gencode"))
    ...
    dp.message.register(cmd_check_mini_wrapper, Command("check_mini"))
    dp.message.register(cmd_get_mini, Command("get_mini"))
    dp.message.register(cmd_debug_mini_wrapper, Command("debug_mini"))
    ...
    # Реєстрація FSM обробника для upload_mini
    async def upload_mini_state_handler(msg: Message, state: FSMContext):
        await upload_mini_handler(msg, bot, state)

    dp.message.register(upload_mini_state_handler, UploadMiniStates.waiting_for_file)
