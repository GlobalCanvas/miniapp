...
async def cmd_status(message: Message):
    ...
    text = (
        ...
        f"📐 <b>Шаблон:</b> <code>{info['width']}x{info['height']}</code> px\n"
...
async def cmd_upload(message: Message):
    ...
    if not message.photo:
        await message.answer(
            ...
            "Прикріпіть зображення до повідомлення з командою!"
        )
        return
    ...
    if len(args) < 2:
        await message.answer(
            "❌ Використання: /upload <x> <y>\n"
            "Приклад: /upload 1000 2000 (з прикріпленим зображенням)"
        )
        return
    ...
    try:
        ...
        text = (
            ...
            f"📐 <b>Розмір:</b> <code>{w}x{h}</code> px\n"
...
async def cmd_get(message: Message):
    ...
    try:
        caption = (
            f"📐 <b>Шаблон чату</b>\n\n"
            f"📍 <b>Координати:</b> <code>{info['x']}, {info['y']}</code>\n"
            f"📐 <b>Розмір:</b> <code>{info['width']}x{info['height']}</code> px\n"
    ...
    try:
        ...
        for player in data.get("ranking", []):
            ...
            if fac_info and len(fac_info) >= 2:
                ...
                if (search_normalized in tag_normalized or 
                    search_normalized in name_normalized or
                    tag_normalized in search_normalized or
...
async def main():
    ...
    dp.message.register(lambda msg: handle_message(msg, chunk_downloader), F.text)
