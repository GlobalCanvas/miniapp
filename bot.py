...
async def cmd_upload(message: Message):
    ...
    # Перевіряємо чи це відповідь на фото
    if not message.reply_to_message or not message.reply_to_message.photo:
        await message.answer(
            "📸 Надішліть PNG файл шаблону з координатами в підписі.\n\n"
            "Формат підпису: X_Y\n"
            "Приклад: -1000_500"
        )
        return
    ...
    # Парсимо координати з підпису
    if not photo_message.caption:
        await message.answer(
            "❌ Не знайдено координати в підписі!\n\n"
            "Формат підпису: X_Y\n"
            "Приклад: -1000_500"
        )
        return
    
    caption = photo_message.caption.strip()
    
    # Парсимо формат X_Y або X Y
    coords = None
    if '_' in caption:
        ...
        # Можливо це просто два числа без роздільника
        import re
